...
const MirarMascota  = () => {
  ...
  const getMascotas = () => {
    axiosClient.get(`/ListarM`).then((response) => {
  ...
  const fetchImagePath = async (imagen) => {
    ...
    const uploadsPath = `http://localhost:3000/ImagenActuaizadas/${imagen}`;
  ...
  const deleteMascotas = (id) => {
    try {
      axiosClient.delete(`/EliminarM/${id}`).then((response) => {
