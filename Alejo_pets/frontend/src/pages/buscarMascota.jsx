...
const ConsultarMascota = () => {
  const { mascota, SetIdMascota } = useHelpsContext();
  const navigate = useNavigate();
  const { id } = useParams();
  const [imagePath, setImage] = useState('');

  useEffect(() => {
    if (typeof SetIdMascota === 'function') {
      SetIdMascota(id);
    } else {
      console.error('SetIdMascota is not a function');
    }
  }, [id]);
  ...
  useEffect(() => {
    if (mascota && mascota.imagen) {
      const Fechimagen = async () => {
        const Actualizaimg = `http://localhost:3000/ImagenActuaizadas/${mascota.imagen}`;
        const img = `http://localhost:3000/img/${mascota.imagen}`;
        ...
        try {
          ...
          if (imgResponse.ok) {
            ...
            const uploadsResponse = await fetch(Actualizaimg, { method: 'HEAD' });
            if (uploadsResponse.ok) {
              setImage(Actualizaimg);
            } else {
              setImage(''); 
            }
          }
